							...
							<div class="col-sm-7">
								<table class="table table-striped">
									...
									<tr>
										...
										<td>
											<div class="input-group">
												<input type="text" class="form-control" autocomplete="off" name="motor_id" id="motor-search" placeholder="Contoh ID Motor : BK001" type="text" value="">
												<span class="input-group-btn">
													<a data-toggle="modal" data-target="#TableMotor" class="btn btn-primary"><i class="fa fa-search"></i></a>
												</span>
											</div>
										</td>
									</tr>
									<tr>
										...
										<td>:</td>
										<td>
											<div id="result_tunggu_motor"> <p style="color:red">* Belum Ada Hasil</p></div>
											<div id="result_motor"></div>
										</td>
									</tr>
								</table>
							</div>
...
<!--modal import -->
<div class="modal fade" id="TableMotor">
<div class="modal-dialog" style="width:80%;">
<div class="modal-content">
<div class="modal-header">
<button type="button" class="close" data-dismiss="modal" aria-label="Close">
<span aria-hidden="true">&times;</span></button>
<h4 class="modal-title">Add Motor</h4>
</div>
...
<table id="example1" class="table table-bordered table-striped">
		...
		<tbody>
		<?php $no=1;foreach($motor->result_array() as $isi){?>
			<tr>
				<td><?= $no;?></td>
				<td><?= $isi['no_rangka'];?></td>
				<td><?= $isi['merk'];?></td>
				...
				<td><?= $isi['tgl_masuk'];?></td>
				<td style="width:17%">
				<button class="btn btn-primary" id="Select_File2" data_id="<?= $isi['motor_id'];?>">
					<i class="fa fa-check"> </i> Pilih
				</button>
				<a href="<?= base_url('data/motordetail/'.$isi['id_motor']);?>" target="_blank">
...
<script>
	$(".fileSelection1 #Select_File2").click(function (e) {
		document.getElementsByName('motor_id')[0].value = $(this).attr("data_id");
		$('#TableMotor').modal('hide');
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('transaksi/motor');?>",
			data:'kode_motor='+$(this).attr("data_id"),
			beforeSend: function(){
				$("#result_motor").html("");
				$("#result_tunggu_motor").html('<p style="color:green"><blink>tunggu sebentar</blink></p>');
			},
			success: function(html){
				$("#result_motor").load("<?= base_url('transaksi/motor_list');?>");
				$("#result_tunggu_motor").html('');
			}
		});
	});
	</script>
	...
	$(document).ready(function(){
		$("#motor-search").keyup(function(){
			$.ajax({
				type: "POST",
				url: "<?php echo base_url('transaksi/motor');?>",
				data:'kode_motor='+$(this).val(),
				beforeSend: function(){
					$("#result_tunggu_motor").html('<p style="color:green"><blink>tunggu sebentar</blink></p>');
				},
				success: function(html){
