...
class Transaksi extends CI_Controller {
	...
	{	
		$this->data['title_web'] = 'Data Pinjam Motor ';
	...
	{	
		$this->data['title_web'] = 'Data Pengembalian Motor ';
		$this->data['idbo'] = $this->session->userdata('ses_id');
	...
	{	
		...
		$this->data['title_web'] = 'Tambah Pinjam Motor ';
	...
	public function detailpinjam()
	{
		...
		$this->data['title_web'] = 'Detail Pinjam Motor ';
		$this->load->view('header_view',$this->data);
		$this->load->view('sidebar_view',$this->data);
	...
	public function kembalipinjam()
	{
		...
		$this->data['title_web'] = 'Kembali Pinjam Motor ';
		$this->load->view('header_view',$this->data);
		$this->load->view('sidebar_view',$this->data);
	...
	public function prosespinjam()
	{
		...
		if(!empty($post['tambah']))
		{
			...
			<p> Tambah Pinjam Motor Sukses !</p>
			</div></div>');
		...
		if($this->input->get('pinjam_id'))
		{
			...
			<p>  Hapus Transaksi Pinjam Motor Sukses !</p>
			</div></div>');
			redirect(base_url('transaksi')); 
		}
		...
		if($this->input->get('kembali'))
		{
			...
			$this->session->set_flashdata('pesan','<div id="notifikasi"><div class="alert alert-success">
			<p> Pengembalian Pinjam Motor Sukses !</p>
			</div></div>');
			redirect(base_url('transaksi')); 
    ...
    {	
		$id = $this->input->post('kode_motor');
		$row = $this->db->query("SELECT * FROM tbl_motor WHERE motor_id ='$id'");
		...
		if($row->num_rows() > 0)
		{
			...
			$item = array(
                ...
                'price'   => '1000',
				'name'    => $tes->merk,
				'options' => array('no_rangka' => $tes->no_rangka,'thn' => $tes->thn_motor,'penerbit' => $tes->penerbit)
			);
	...
	?>
		<table class="table table-striped">
			...
			<?php $no=1;
				...
				<tr>
					...
					<td style="width:17%">
					<a href="javascript:void(0)" id="delete_motor<?=$no;?>" data_<?=$no;?>="<?= $items['id'];?>" class="btn btn-danger btn-sm">
						<i class="fa fa-trash"></i></a>
					</td>
				</tr>
				<script>
					$(document).ready(function(){
						$("#delete_motor<?=$no;?>").click(function (e) {
							$.ajax({
								type: "POST",
								url: "<?php echo base_url('transaksi/del_cart');?>",
								data:'kode_motor='+$(this).attr("data_<?=$no;?>"),
								beforeSend: function(){
								},
		...
		<?php foreach(array_values(unserialize($this->session->userdata('cart'))) as $items){?>
			<input type="hidden" value="<?= $items['id'];?>" name="idmotor[]">
		<?php }?>
		<div id="tampil"></div>
	<?php
	}
	...
	public function del_cart()
    {
       ...
       // redirect('jual/tambah');
		echo '<script>$("#result_motor").load("'.base_url('transaksi/motor_list').'");</script>';
    }
